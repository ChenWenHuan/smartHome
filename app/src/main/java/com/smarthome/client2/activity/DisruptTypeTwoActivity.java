...
public class DisruptTypeTwoActivity extends Activity
{

    public final static String DISRUPT_TYPE_2_FILTER = "com.smarthome.client2.disrupttime_2";
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_disrupt_type_2);
        initWidget();
        addTopBarToHead();
    ...
    private void initWidget()
    {
        ...
        gprs_list_tv.setOnClickListener(new OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                startActivity(new Intent(DisruptTypeTwoActivity.this,
                        DisruptTypeTwoTimePickActivity.class));
            }
        });
        ...
        registerReceiver(gprsReceiver, new IntentFilter(DISRUPT_TYPE_2_FILTER));
                ...
                new OnClickListener()
                {
                    ...
                    public void onClick(View v)
                    {
                        ...
                        AlertDialog alertDialog = new AlertDialog.Builder(
                                DisruptTypeTwoActivity.this).setOnKeyListener(new OnKeyListener()
                        {
                            @Override
                                ...
                                .setNegativeButton(getString(R.string.common_btn_yes),
                                        new DialogInterface.OnClickListener()
                                        {
                                                    ...
                                                    int which)
                                            {
                                                if (gprs_ischeck.isChecked())
                                                {
                                                    setSilenceFromServer(Preferences.getInstance(getApplicationContext())
                                                            .getDeviceId(),
                                                            1);
                                                    status = 1;
                                                }
        ...
        public void handleMessage(android.os.Message msg)
        {
            ...
            switch (msg.what)
            {
                case Constants.GET_DATA_START:
                    dialog = new ProgressDialog(DisruptTypeTwoActivity.this);
                            ...
                            try
                            {
                                ...
                                if (array.length() != 0)
                                {
                                    Preferences.getInstance(getApplicationContext())
                                            .setDisruptNum(workd + "-" + satd
                                                    + "-" + sund);
                                    Preferences.getInstance(getApplicationContext())
                                            .setDisturbTime(gpsTimes.substring(2));
                                }
                                else
                                {
                                    Preferences.getInstance(getApplicationContext())
                ...
                case Constants.GET_DATA_FAIL:
                    if (dialog != null)
                    {
                        dialog.dismiss();
                        Toast.makeText(getApplicationContext(),
                                getString(R.string.disrupt_receive_info_fail),
                                Toast.LENGTH_SHORT).show();
                    }
                    break;
    ...
    private void setSilenceFromServer(int deviceId, int status)
    {
        if (!HttpUtil.isNetworkAvailable(getApplicationContext()))
        {
            Toast.makeText(getApplicationContext(),
    ...
    private BroadcastReceiver gprsReceiver = new BroadcastReceiver()
    {
        ...
        {
            if (intent.getAction().equals(DISRUPT_TYPE_2_FILTER))
            {
                //                getGPRSFromServer();
                gprsTime.clear();
                workd = 0;
    ...
    public void finish()
    {
        ...
        Preferences.getInstance(getApplicationContext()).setDisruptNum("");
        super.finish();
    }
